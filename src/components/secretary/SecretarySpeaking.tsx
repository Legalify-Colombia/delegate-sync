...
export default function SecretarySpeaking({ committeeId }: SecretarySpeakingProps) {
  ...
  const startSpeaking = async () => {
    ...
    // First, end any existing active speaking session
    if (speakingStatus?.is_active) {
      await endSpeaking();
    }
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>Intervención del Secretario</span>
        </CardTitle>
        <CardDescription>
          Controla tu turno para dirigirte al comité
        </CardDescription>
      </CardHeader>
